...
Function GetVideo(id as String, urlParams = {} As Object) As Object
  ...
  if response <> invalid
    data = response.response
    urlParams1 = {}
    urlParams1.AddReplace("active", true)
    urlParams1.AddReplace("video_id", id)
    urlParams1.AddReplace("category", "Creator Name")
    params1 = AppendAppKeyToParams(urlParams1)
    response1 = CreatorForVideo(params1)
    if response1 <> invalid
      data.AddReplace("creator", response1[0].title)
      ?"the GetVideo is ==>"data
    end if
  end if
...
Function CreatorForVideo(urlParams = {} As Object) As Object
  data = {}
  ...
  response = MakeRequest(url, params)
  if response <> invalid
    ?"the CreatorForVideo is creator ==>"response.response[0]
    data = response.response
  end if
...
function GetPlaylistVideos(playlist_id as String, urlParams = {} as Object) as Object
  ...
  if response <> invalid
    data = response.response
    ?"GetPlaylistVideos==>"data[0]._id
    videos = []
    for each video in data
      urlParams = {}
      urlParams.AddReplace("active", true)
      urlParams.AddReplace("video_id", video._id)
      urlParams.AddReplace("category", "Creator Name")
      params = AppendAppKeyToParams(urlParams)
      response = CreatorForVideo(params)
      video.AddReplace("creator", response[0].title)
      videos.push(video)
    end for
  end if
